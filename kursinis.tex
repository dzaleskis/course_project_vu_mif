...
\begin{algorithm}
  \caption{Vadovėlinis Šelo rikiavimo algoritmas}\label{css} % TO may need fixing
  \begin{algorithmic}[1]
  \ForEach {$gap$ in $H$}
    \For {$i\gets gap+1$ to $N$} % TO may need fixing
      \State $j\gets i$
      \State $temp\gets S[i]$
      \While {$j > gap$ and $S[j - gap] > S[j]$}
